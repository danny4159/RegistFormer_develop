...
class LapIRN_Lv2_Module(BaseModule_Registration):
    def __init__(
        self,
        netR_2: torch.nn.Module, # TODO: Miccai2020_LDR_laplacian_unit_disp_add_lvl1(2, 3, start_channel=start_channel, is_train=True, imgshape=imgshape_4,range_flow=range_flow)
        optimizer,
        params,
        # scheduler,
        *args,
    ...
    def backward_R_lv2(self, displacement_field, warped_img, down_fixed_img): # grid_4 
        loss_multiNCC = self.loss_similarity_multi_res(warped_img, down_fixed_img)
        ...
        loss_lv2 = loss_multiNCC + self.params.lambda_smooth * loss_regulation
        return loss_lv2
    ...
    def model_step(self, batch: Any, is_3d=False, is_train=False):
        ...
        if is_3d:
            ...
            deform_field, warped_img, down_fixed_img, _, _, _ = self.netR_2(moving_img, fixed_img) #  F_X_Y, X_Y, Y_4x, F_xy, _ = deform_field, warped_img, down_fixed_img, displacement field, feature map
    ...
    def training_step(self, batch: Any, batch_idx: int):
        ...
        with optimizer_R_A.toggle_model():
            loss = self.backward_R_lv2(deform_field, warped_img, down_fixed_img)
    ...
    def configure_optimizers(self):
        ...
        optimizer_R_A = self.hparams.optimizer(params=self.netR_2.parameters())
    ...
    def on_epoch_start(self):
        ...
        if current_epoch == 3:
            self.netR_2.unfreeze_modellvl1()