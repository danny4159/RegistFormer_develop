...
def finetune_execute(model, image_A, image_B, steps):
    ...
    for _ in range(steps):
        ...
        loss, _, _ = model(image_A, image_B)
        # print(loss_tuple)
        # loss_tuple[0].backward()
        finetune_execute.backward()
        optimizer.step()
    with torch.no_grad():
...
def register_pair(
    ...
    A_trch = torch.Tensor(A_npy).to(model_device)[None, None]
    B_trch = torch.Tensor(B_npy).to(model_device)[None, None]

    shape = model.fullres_net.identity_map.shape
    ...
    if finetune_steps == None:
        with torch.no_grad():
            loss, _, _ = model(A_resized, B_resized)
    else:
    ...
    phi_AB = model.fullres_net.phi_AB(model.fullres_net.identity_map)
    phi_BA = model.fullres_net.phi_BA(model.fullres_net.identity_map)

    # the parameters ident, image_A, and image_B are used for their metadata
    itk_transforms = (
        create_itk_transform(phi_AB, model.fullres_net.identity_map, image_A, image_B),
        create_itk_transform(phi_BA, model.fullres_net.identity_map, image_B, image_A),
    )
...
def register_pair_with_multimodalities(
    ...
    model_device = next(model.parameters()).device
    A_trch = torch.Tensor(np.array(A_npy).to(model_device))[None]
    B_trch = torch.Tensor(np.array(B_npy).to(model_device))[None]

    shape = model.fullres_net.identity_map.shape[2:]
    ...
    phi_AB = model.phi_AB(model.fullres_net.identity_map)
    phi_BA = model.phi_BA(model.fullres_net.identity_map)

    # the parameters ident, image_A, and image_B are used for their metadata
    itk_transforms = (
        create_itk_transform(phi_AB, model.fullres_net.identity_map, image_A[0], image_B[0]),
        create_itk_transform(phi_BA, model.fullres_net.identity_map, image_B[0], image_A[0]),
    )
    if not return_artifacts:
        return itk_transforms
    else:
