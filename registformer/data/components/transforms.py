...
):  # TODO: Adaconv 쓸 시 수정 adaconv -> min_size=(592,384) (16배수), 원래 256,256 (나머지 다)
    """
...
def JHL_deformation(A: np.ndarray, B: np.ndarray, deform_prob: float):
...
def JHL_motion_artifacts(A: np.ndarray, B: np.ndarray, motion_prob: float):
...
def JHL_rotate_images(A: np.ndarray, B: np.ndarray, degree: float):
...
def translate_images(
    A: torch.Tensor, B: torch.Tensor, misalign_x: float, misalign_y: float
):
...
def download_process_SynthRAD_MR_CT_Pelvis(
    data_dir: str,  # h5를 만들기위한 함수. nifti -> h5로 해야해.
    write_dir: str,
    misalign_x: float = 0.0,  # maximum misalignment in x direction (float)
    misalign_y: float = 0.0,  # maximum misalignment in y direction (float)
    degree: float = 0.0,  # maximum rotation in z direction (float)
...
class dataset_SynthRAD_MR_CT_Pelvis_RAM(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.rand_crop:
            ...
            A, B = random_crop(
                A, B, (h // 4 * 4, w // 4 * 4)
            )  # under nearest multiple of four
